...
export async function getMessageValue(network: NetworkType): Promise<string> {
  ...
  try {
    const response = await fetch(
      `${apiUrl}/v2/contracts/call-read/${CONTRACT_ADDRESS}/${CONTRACT_NAME}/get-message`,
      {
        method: 'POST',
    ...
    if (response.ok) {
      ...
      try {
        const devnetResponse = await fetch(
          `http://localhost:3999/v2/contracts/call-read/${CONTRACT_ADDRESS}/${CONTRACT_NAME}/get-message`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
